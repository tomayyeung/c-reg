...
export default function CourseCatalog() {
  ...
  const fetchCourses = async (page = 1) => {
    try {
      ...
      if (prerequisitesFilter) params.append("hasPrerequisites", prerequisitesFilter)
      ...
      <div className={styles["catalog-layout"]}>
        <div className={styles["filters-container"]}>
          <div className={styles.card}>
            ...
            <div className={styles["card-content"]}>
              ...
              <div className={styles["filter-group"]}>
                <label className={styles["filter-label"]} htmlFor="restrictions">
                  Restrictions
                </label>
                <input
                  id="restrictions"
              ...
              <div className={styles["filter-group"]}>
                <label className={styles["filter-label"]} htmlFor="prerequisites">
                  Prerequisites
                </label>
                <select
                  id="prerequisites"
              ...
              <button className={styles["filter-button"]} onClick={handleSearch}>
                Apply Filters
              </button>

              <button className={`${styles["filter-button"]} ${styles.secondary}`} onClick={handleReset}>
                Reset Filters
              </button>
            </div>
          </div>
        ...
        <div className={styles["courses-container"]}>
          <div className={styles.card}>
            ...
            <div className={styles["card-content"]}>
              {loading ? (
                <div className={styles["loading-message"]}>Loading courses...</div>
              ) : courses.length === 0 ? (
                <div className={styles["empty-message"]}>No courses found matching your criteria.</div>
              ) : (
                      ...
                      <div className={styles["course-header"]}>
                        <h3 className={styles["course-title"]}>
                          ...
                          <span className={styles["units-badge"]}>
                            {course.units} {Number.parseInt(course.units) === 1 ? "Unit" : "Units"}
                          </span>
                        </h3>
                        <p className={styles["course-code"]}>{course.course_code}</p>
                        <p className={styles["course-college"]}>{course.college}</p>
                      </div>
                      <div className={styles["course-details"]}>
                        <div className={styles["course-section"]}>
                          ...
                          <p className={styles["section-content"]}>
                            {course.description || (
                              <span className={styles["empty-value"]}>No description available</span>
                            )}
                          </p>
                        </div>
                        ...
                        {course.prerequisites && (
                          <div className={styles["course-section"]}>
                            <h4 className={styles["section-title"]}>Prerequisites</h4>
                            <p className={styles["section-content"]}>{course.prerequisites}</p>
                          </div>
                        )}

                        {course.restrictions && (
                          <div className={styles["course-section"]}>
                            <h4 className={styles["section-title"]}>Restrictions</h4>
                            <p className={styles["section-content"]}>{course.restrictions}</p>
                          </div>
                        )}
                      </div>
                    </div>
